						...
						<c:forEach items="${helpMenus}" var="helpMenu" varStatus="index">
							<li>
								<c:if test="${helpMenu.size()==1}">
									<a href="${ctx}/front/helpCenter?id=${helpMenu[0].id}" id="helpMenu${helpMenu[0].id}">${helpMenu[0].name}</a>
								</c:if>
								<c:if test="${helpMenu.size()>1}">
					                ...
					                <dl class="hl-sec-nav">
										<c:forEach items="${helpMenu}" var="helpMenuSon" varStatus="indexSon">
											<c:if test="${indexSon.index!=0}">
												<dd><a href="${ctx}/front/helpCenter?id=${helpMenuSon.id}" id="helpMenu${helpMenuSon.id}"><em></em><tt>${helpMenuSon.name}</tt></a></dd>
											</c:if>
										</c:forEach>
									</dl>
								</c:if>
